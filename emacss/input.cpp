...
void node::input(int argc, char* argv[]){
  ...
  while (( c = getopt 
    ...
    case ('v'):                              //Cluster orbital velocity (kms^-2)
      ...
      break;
    case ('o'):                              //Output setting (N-body/real units)
      value = optarg;
      units = atoi(value);
      break;
...
void node::initialise(){
  ...
  double G = 4.302e-3;    //pc M_sun^_1 (km^2)(s^-2)
  if (galaxy.M.Msun == 0 && galaxy.R.pc != 0 && galaxy.v.kms != 0) 
    galaxy.M.Msun = (pow(galaxy.v.kms,2)*galaxy.R.pc)/(2.0*G);
  else if (galaxy.M.Msun != 0 && galaxy.R.pc == 0 && galaxy.v.kms != 0) 
    galaxy.R.pc = (2.0*G*galaxy.M.Msun)/pow(galaxy.v.kms,2);
  else if (galaxy.M.Msun != 0 && galaxy.R.pc != 0 && galaxy.v.kms == 0) 
    galaxy.v.kms = pow((2.0*G*galaxy.M.Msun)/galaxy.R.pc,1.0/2.0);
  else{
  ...
  //Sets final factors needed - mean mass, E, trh and rj
  mm.nbody = 1.0/N; 
