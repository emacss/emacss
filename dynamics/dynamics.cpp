...
double dynamics::drcdt(){                               //This is not used
  double drcdt = 0;
  drcdt = delta()*mynode->rc.nbody/mynode->t_rhp.nbody;
  return drcdt;
}
...
double dynamics::gamma_dyn(){                   //Equation (??) AGL2013
  ...
  if (mynode->galaxy.type > 0)
    X += (mynode->mm.nbody - mynode->m_min.nbody)*F+mynode->m_min.nbody;
  cerr << X << endl;
  gamma = (1.0-X)*xi();
  return gamma;
}
...
double dynamics::delta(){
  double delta = 0, F1 = 0;
    
  if (mynode->trhp < mynode->T_DYN){
      delta = d1+d2*(mynode->t_relax.nbody/mynode->t_rc.nbody);
  }
  else{
      F1 = (2.0/3.0)*xi_e()/(1+mynode->N/N3)*(1+K());
      cerr << xi() << endl;
      delta = F1-2.0*xi_e()+myse->zeta();
      cerr << delta << endl;
  }
	
  return delta/myse->zeta();
}
...
double dynamics::K(){				 //Equation (29) GALB2013          
  ...
  K = 2.0*mynode->rcrh*(k0-k1)*\
	exp(-pow(mynode->rcrh/Rch0,2.0))/(sqrt(M_PI)*Rch0*k());
  return K;
}
...
double dynamics::k(){				 //Breakdown, GALB2013           
  ...
  k = (k1+(k0-k1)*erf(mynode->rcrh/Rch0));
  return k;
}
