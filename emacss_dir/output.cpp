...
//    F = -se.sigma_R();
	...
	cerr << endl;
        ...
        if (units > 0){
	    printf("#1r t[Myr]\tN\t\tM[M_sun]\tr[pc]\t\trj[pc]\t\trc[pc]\n");
	    printf("#2r trh[Myr]\tn_relax\t\tE[Real]\t\tMS-factor\tkappa\n");  
	}
	if (units != 1){
	    printf("#1n t\t\tN\t\tM\t\tr\trj\trc\n");
    ...
    if (out_time.Myr == numeric_limits<double>::max()){
	if (units > 0){
	    printf("#1r %8.4e\t%8.4e\t%8.4e\t%8.4e\t%8.4e\t%8.4e\n",
		  time.Myr,N,N*mm.Msun,r.pc,rj.pc,rc.pc);
	    printf("#2r %8.4e\t%8.4e\t%8.4e\t%8.4e\t%8.4e\n",
		  t_relax.Myr,trhelapsed,E.real,mass_seg,kappa);
	}
	if (units != 1){
	    printf("#1n %8.4e\t%8.4e\t%8.4e\t%8.4e\t%8.4e\t%8.4e\n",
		  time.nbody,N,N*mm.nbody,r.nbody,rj.nbody,rc.nbody);
    ...
    if (time.nbody > out_time.nbody && units > 0){
	    printf("\n#1r  %8.4e\t%8.4e\t%8.4e\t%8.4e\t%8.4\t%8.4e\n",
		  time.Myr,N,N*mm.Msun,r.pc,rj.pc,rc.pc);
	    printf("#2r  %8.4e\t%8.4e\t%8.4e\t%8.4e\t\t%8.4e\n",
		  t_relax.Myr,trhelapsed,E.real,mass_seg,kappa);
    }
    if (time.nbody > out_time.nbody && units != 1){
	    printf("\n#1n  %8.4e\t%8.4e\t%8.4e\t%8.4e\t%8.4\t%8.4e\n",
		    time.nbody,N,N*mm.nbody,r.nbody,rj.nbody,rc.nbody);
	    printf("#2n  %8.4e\t%8.4e\t%8.4e\t%8.4e\t\t%8.4e\n",
		  t_relax.nbody,trhelapsed,E.nbody,mass_seg,kappa);
    }
