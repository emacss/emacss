...
{
  double sigc2 = 8./3.*M_PI*rhoc()*pow(rc.nbody,2.0); // Core velocity dispersion
  return pow(sigc2,1.5)/(15.4*mm.nbody*rhoc()*log(0.02*N));
}

double node::rhoc() // MGi: compute core density
{
  double rhoc = 0;
  if (trhp < T_DYN) rhoc = 0.055*pow(rc.nbody,-2.2);
  else{
    double rhoh = 3.0*N*mm.nbody/(8*M_PI*pow(r.nbody,3.0));
    rhoc = pow(rc.nbody/r.nbody,-2.0)*rhoh;
  }
  return rhoc;
}
...
void node::evolve(stellar_evo se,dynamics dyn){
  ...
  static double duplicate_nbody[13]; 
...
void node::convert(stellar_evo se){
 /*Housekeeping function. Keeps n-body and real units aligned when called.
  Also calls functions to determine the quantities that need to be redefined*/
    ...
    t_rc.nbody = trc(); t_rc.Myr = t_rc.nbody*T_star;  
    rj.nbody = r_jacobi(); rj.pc = rj.nbody*R_star;
    rhrj = r.nbody/rj.nbody;
    if (rc.nbody < 0.01 || rc.nbody != rc.nbody ) rc.nbody = 0.01;
    rc.pc = rc.nbody*R_star;
    rcrh = rc.nbody/r.nbody;
